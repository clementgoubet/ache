...
public class LinkStorage extends StorageDefault {
	...
	public LinkStorage(LinkStorageConfig config, BipartiteGraphManager manager, FrontierManager frontierManager)
			throws IOException {
	...
	public synchronized Object insert(Object obj) throws StorageException {
		...
		try {
			...
			/*
			 * lms = graphManager.getRepository().getLMs(); for(int i=0;
			 * i<lms.length; i++){ if(lms[i].getIsTargetOfBacklink())
		...
		try {
			...
			if (nextURL.getType() == LinkRelevance.TYPE_BACKLINK_BACKWARD) {
				try {
					...
					// recursive call to get the next URL to be fed to the
					// downloader
					return this.select(obj);
				}
			} else if (nextURL.getType() != LinkRelevance.TYPE_FORWARD
					&& nextURL.getType() != LinkRelevance.TYPE_BACKLINK_FORWARD) {
	...
	public static Storage createLinkStorage(String configPath, String seedFile, String dataPath, String modelPath,
			LinkStorageConfig config) throws LinkClassifierFactoryException, FrontierPersistentException, IOException {
		...
		LinkClassifier outlinkClassifier = linkClassifierFactory
				.createLinkClassifier(config.getTypeOfOutlinkClassifier());
		LinkClassifier backlinkForwardClassifier = linkClassifierFactory
				.createLinkClassifier(config.getTypeOfBacklinkForwardClassifier());
		LinkClassifier backlinkBackwardClassifier = linkClassifierFactory
				.createLinkClassifier(config.getTypeOfBacklinkBackwardClassifier());
		...
		BipartiteGraphManager manager = createBipartiteGraphManager(config, outlinkClassifier,
				backlinkForwardClassifier, backlinkBackwardClassifier, frontierManager, graphRep);
		...
		if (config.isUseOnlineLearning()) {
			...
			LinkClassifierBuilder cb = new LinkClassifierBuilder(graphRep, stoplist, wrapper,
					frontierManager.getLinkFrontier(), modelPath);

			logger.info("ONLINE LEARNING:" + config.getOnlineMethod());
			OnlineLearning onlineLearning = new OnlineLearning(frontierManager.getLinkFrontier(),
					frontierManager.getBacklinkFrontier(), manager, cb, config.getOnlineMethod(),
					dataPath + "/" + config.getTargetStorageDirectory());
	...
	private static BipartiteGraphManager createBipartiteGraphManager(LinkStorageConfig config,
			LinkClassifier outlinkClassifier, LinkClassifier backlinkForwardClassifier,
			LinkClassifier backlinkBackwardClassifier, FrontierManager frontierManager,
		...
		if (config.getBacklinks()) {
			...
			manager = new BipartiteGraphManager(frontierManager, graphRep, outlinkClassifier, backlinkForwardClassifier,
					backlinkBackwardClassifier);
